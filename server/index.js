...
if (!apiKey) {
	console.warn('GOOGLE_API_KEY is not set. The career gemini endpoint will fail until this is configured.');
}

const genAI = apiKey ? new GoogleGenerativeAI(apiKey) : null;
const model = genAI ? genAI.getGenerativeModel({ model: 'gemini-3-flash-preview' }) : null;

app.post('/api/career-gemini', async (req, res) => {
	if (!model) {
	...
	try {
		...
		const systemPrompt = `You are a friendly, concise \"Career Gemini\" for ${profile.name}.
...
`;
		...
		console.error('Error in /api/career-gemini:', err);
